...
###############################################
#Description : Debian 12 GestSup install script
#Update : 09/02/2024
#Version : 2.4
###############################################
...
service apache2 start -y &> /dev/null
...
service mariadb start -y &> /dev/null
...
echo "3 - Install PHP "
echo -n "      > install PHP : "
apt install php libapache2-mod-php -y &> /dev/null
apt install php-{common,curl,gd,imap,intl,ldap,mbstring,mysql,xml,zip} -y &> /dev/null
...
mariadb -e "DROP USER IF EXISTS 'gestsup'@'localhost';"
mariadb -e "CREATE USER 'gestsup'@'localhost' IDENTIFIED BY '$db_gestsup_password';"
mariadb -e "GRANT ALL PRIVILEGES ON *.* TO 'gestsup'@'localhost';"
...
sed -i 's/max_execution_time = 30/max_execution_time = 480/g' /etc/php/8.2/apache2/php.ini &> /dev/null
echo -e "\e[32mOK\e[0m"
echo -n "      > memory_limit : 512M "
sed -i 's/memory_limit = 128M/memory_limit = 512M/g' /etc/php/8.2/apache2/php.ini &> /dev/null
echo -e "\e[32mOK\e[0m"
echo -n "      > upload_max_filesize : 8M "
sed -i 's/upload_max_filesize = 2M/upload_max_filesize = 8M/g' /etc/php/8.2/apache2/php.ini &> /dev/null
echo -e "\e[32mOK\e[0m"
echo -n "      > date.timezone : Europe/Paris "
sed -i 's/;date.timezone =/date.timezone = Europe\/Paris/g' /etc/php/8.2/apache2/php.ini &> /dev/null
echo -e "\e[32mOK\e[0m"
...
sed -i "s/# this is only for embedded server/skip-name-resolve/g" /etc/mysql/mariadb.conf.d/50-server.cnf &> /dev/null
echo -e "\e[32mOK\e[0m"
echo -n "      > innodb_buffer_pool_size : $ram_to_configure_mb "
sed -i "s/#innodb_buffer_pool_size = 8G/innodb_buffer_pool_size = $ram_to_configure_mb/g" /etc/mysql/mariadb.conf.d/50-server.cnf &> /dev/null
echo -e "\e[32mOK\e[0m"

echo -n "7 - Download GestSup : "
last_version=`curl -s https://gestsup.fr/lastest_version.php`
wget -P /var/www/html https://gestsup.fr/downloads/versions/current/version/gestsup_$last_version &> /dev/null
echo -n "gestsup_$last_version "
echo -e "\e[32mOK\e[0m"

echo -n "8 - Extract files : "
unzip -o /var/www/html/gestsup_$last_version -d /var/www/html &> /dev/null
echo -e "\e[32mOK\e[0m"

echo "9 - Remove install files "
echo -n "      > gestsup_$last_version : "
if test -f "/var/www/html/gestsup_$last_version"; then
    rm /var/www/html/gestsup_$last_version
fi
echo -e "\e[32mOK\e[0m"
echo -n "      > index.html : "
...
/etc/init.d/mariadb start &> /dev/null
mariadb -e "CREATE DATABASE IF NOT EXISTS bsup;"
echo -n 'bsup '
echo -e "\e[32mOK\e[0m"

echo -n "11 - Import database skeleton : "
mariadb  bsup < /var/www/html/_SQL/skeleton.sql  &> /dev/null
mariadb  bsup -e "UPDATE tparameters SET telemetry='1'"
mariadb  bsup -e "UPDATE tparameters SET user_password_policy='1'"
mariadb  bsup -e "UPDATE tparameters SET user_password_policy_min_lenght='8'"
mariadb  bsup -e "UPDATE tparameters SET server_url='http://$ip'"
mariadb  bsup -e "UPDATE tparameters SET server_date_install='$current_date'"
server_private_key=`tr -dc A-Za-z0-9 </dev/urandom | head -c 32 ; echo ''`
mariadb  bsup -e "UPDATE tparameters SET server_private_key='$server_private_key'"
...
mariadb  bsup -e "UPDATE tusers SET password='$admin_password_hash', last_pwd_chg='$current_date'  WHERE login='admin'"

password=`tr -dc A-Za-z0-9 </dev/urandom | head -c 32 ; echo ''`
password_hash=`php -r "echo password_hash('$password', PASSWORD_DEFAULT);"`
mariadb  bsup -e "UPDATE tusers SET password='$password_hash', last_pwd_chg='$current_date'  WHERE login='super'"

password=`tr -dc A-Za-z0-9 </dev/urandom | head -c 32 ; echo ''`
password_hash=`php -r "echo password_hash('$password', PASSWORD_DEFAULT);"`
mariadb  bsup -e "UPDATE tusers SET password='$password_hash', last_pwd_chg='$current_date'  WHERE login='tech'"

password=`tr -dc A-Za-z0-9 </dev/urandom | head -c 32 ; echo ''`
password_hash=`php -r "echo password_hash('$password', PASSWORD_DEFAULT);"`
mariadb  bsup -e "UPDATE tusers SET password='$password_hash', last_pwd_chg='$current_date'  WHERE login='poweruser'"

password=`tr -dc A-Za-z0-9 </dev/urandom | head -c 32 ; echo ''`
password_hash=`php -r "echo password_hash('$password', PASSWORD_DEFAULT);"`
mariadb  bsup -e "UPDATE tusers SET password='$password_hash', last_pwd_chg='$current_date'  WHERE login='user'"

echo -e "\e[32mOK\e[0m"

echo -n "13 - Update connect file : "
sed -i 's/db_name=\x27/db_name=\x27bsup/g' /var/www/html/connect.php &> /dev/null
sed -i 's/user=\x27/user=\x27gestsup/g' /var/www/html/connect.php &> /dev/null
sed -i 's/password=\x27/password=\x27'$db_gestsup_password'/g' /var/www/html/connect.php &> /dev/null
echo -e "\e[32mOK\e[0m"
...
then
        ...
        for ((i=last_version_number_short+1;i<=last_patch_number_short;i++)); do
            ...
            #download patch
            wget -P /var/www/html https://gestsup.fr/downloads/versions/current/stable/patch_$patch_to_install.zip &> /dev/null
            if test -f "/var/www/html/patch_$patch_to_install.zip"
            then
                #extract patch files
                unzip -o /var/www/html/patch_$patch_to_install.zip -d /var/www/html &> /dev/null
                #remove patch zip file
                rm /var/www/html/patch_$patch_to_install.zip* &> /dev/null
                #update database
                ...
                mariadb  bsup < $update_db_file  &> /dev/null
                echo -e "\e[32mOK\e[0m"
            else 
...
adduser gestsup --ingroup www-data &> /dev/null
chown -R gestsup:www-data /var/www/html/ &> /dev/null
find /var/www/html/ -type d -exec chmod 750 {} \; &> /dev/null
find /var/www/html/ -type f -exec chmod 640 {} \; &> /dev/null
chmod 770 -R /var/www/html/upload &> /dev/null
chmod 770 -R /var/www/html/images/model &> /dev/null
chmod 770 -R /var/www/html/backup &> /dev/null
chmod 770 -R /var/www/html/_SQL &> /dev/null
echo -e "\e[32mOK\e[0m"

echo "16 - Update Apache default parameters "
echo -n "      > ServerSignature : Off "
sed -i 's/ServerSignature On/ServerSignature Off/g' /etc/apache2/conf-available/security.conf &> /dev/null
echo -e "\e[32mOK\e[0m"
echo -n "      > ServerTokens : Prod "
sed -i 's/ServerTokens OS/ServerTokens Prod/g' /etc/apache2/conf-available/security.conf &> /dev/null
echo -e "\e[32mOK\e[0m"
echo -n "      > Remove Indexes from www directory "
sed -i 's/Options Indexes FollowSymLinks/Options FollowSymLinks/g' /etc/apache2/apache2.conf &> /dev/null
echo -e "\e[32mOK\e[0m"
